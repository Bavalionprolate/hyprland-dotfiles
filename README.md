...
> Индикатор звука поломон, из-за ошибки с функцией `eww update` для `defvar`, если вы уверены что у вас будет работать правильно eww, то можете из ветки `V1` извлечь папку для индикатора и поместить её в `~/.config/eww`
...
* Используя комбинацию клавиш `Win + Tab`, вы сможете вызвать Overview и просмотреть все приложения на всех workspaces. Переключить фокус в overview вы можете через комбинации клавиш `ALT + Tab`,`Win + Arrows` и что бы переместиться к свокусированному приложению вы можете нажав `левой кнопки мыши` или нажав еще раз `Win + Tab`.
...
* Для перемещения приложения в special workspace можно нажав комбинацию `Win + Shift + C`, если вы хотите переместить приложение и просматривать special workspace, то используйте `Win + C`. Для того что бы просто просмотреть special workspace, вы можете использовать `Win + S` и так же если необходимо выйти из этого пространства.
...
* Для того что бы переключить в плавующий режим определенное окно, вам нужно что бы оно было сфокусированное и по сочитанию `Win + Shift + E` вы сможете изменить режим окна, так же можно и убрать плавующий режим для окна.

* Для того что бы переключить в плавующий режим целое пространство, вам нужно использовать сочетание клавиш `Win + E`, так же можно и убрать плавующий режим.
